...
export default function DashboardTab({ user, composers, concerts, posts, onRequireAuth, onUpdateUser }: DashboardTabProps) {
  ...
  const handleTestConnection = async () => {
    if (!supabaseUrl || !supabaseAnonKey) {
      setTestResult({ success: false, message: "URLとAnon Keyを先に入力してください。" });
      return;
    }
    ...
    try {
      ...
      setTestResult({ success: false, message: `例外エラーが発生しました: ${e.message || e}` });
    } finally {
  ...
  const handleSaveDashboard = async () => {
    ...
    if (res.success) {
      ...
      refreshDashboardList();
      setTimeout(() => setSaveStatus({ type: "", message: "" }), 6000);
    } else {
      setSaveStatus({
        type: "error",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
            ...
            <form onSubmit={handleConnectSupabase} className="space-y-3.5">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  type="submit"
                  className="bg-yellow-100/90 hover:bg-yellow-50 text-stone-950 font-bold text-3xs py-2 px-3 rounded-lg transition"
                >
                  保存
                </button>
                <button
                  ...
                  className="flex-1 bg-stone-900 hover:bg-stone-850 border border-stone-800 text-stone-300 font-bold text-3xs py-2 rounded-lg transition disabled:opacity-50 flex items-center justify-center gap-1"
                >
                  {isTestingConnection ? "テスト中..." : "接続テストを実行"}
                </button>
                {(supabaseUrl || supabaseAnonKey) && (
                  <button
                    type="button"
                    onClick={handleClearKeys}
                    className="bg-stone-950 hover:bg-stone-850 border border-stone-800 text-stone-400 text-3xs px-2.5 py-2 rounded-lg transition"
                  >
                    初期化
                  </button>
                )}
              ...
              {testResult && (
                <div className={`p-3 rounded-lg text-[10px] leading-relaxed border flex items-start gap-1.5 ${
                  testResult.success 
                  ...
                  {isViewingSelf ? (
                    ...
                    <div className="space-y-2">
                      <div className="p-3 bg-stone-900/30 rounded-xl border border-stone-850">
                        <span className="text-[8px] text-amber-400 block font-bold font-mono">2026-11-08 (19:00)</span>
                        <h5 className="text-3xs font-bold text-stone-200 mt-1">ウィーン・フィルハーモニー管弦楽団来日公演</h5>
                        <p className="text-[9px] text-stone-500 mt-1">モーツァルト：交響曲 第41番『ジュピター』、ブルックナー：交響曲 第7番 / 指揮: クリスティアン・ティーレマン</p>
                      </div>
                    </div>
                  )}
                </div>
